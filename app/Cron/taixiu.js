...
let GetTop = function(){
	TaiXiu_User.find({'totall':{$gt:0}}, 'totall uid', {sort:{totall:-1}, limit:10}, function(err, results) {
		if (err) {
			console.error('[GetTop] MongoDB query failed:', err.message || err);
			io.listTop = [];
			return;
		}
		...
		Promise.all(results.map(function(obj){
			return new Promise(function(resolve) {
				UserInfo.findOne({'id': obj.uid}, function(error, result2){
					if (error) {
						console.error('[GetTop] UserInfo query failed:', error.message || error);
					}
					resolve({
		...
		.catch(function(error){
			console.error('[GetTop] failed:', error.message || error);
			io.listTop = [];
		});
	});
};
...
let TopHu = function(){
	HU_game.find({}, 'game type red bet toX balans x').exec(function(err, data){
		if (err) {
			console.error('[TopHu] MongoDB query failed:', err.message || err);
			return;
		}
...
let setTaiXiu_user = function(phien, dice){
	TXCuocOne.find({phien:phien}, {}, function(err, list) {
		if (err) {
			console.error('[setTaiXiu_user] MongoDB query failed:', err.message || err);
			return;
		}
...
let thongtin_thanhtoan = function(game_id, dice = false){
	if (dice) {
		...
		TXCuoc.find({phien:game_id}, null, {sort:{'_id':-1}}, function(err, list) {
			if (err) {
				console.error('[thongtin_thanhtoan] MongoDB query failed:', err.message || err);
				if (dice) playGame();
				return;
			}
			if (!Array.isArray(list)) {
					...
					if (obj.select === true){ // Tổng Red Tài
						...
						if (TaiXiu_red_lech_tai && TaiXiu_tong_red_lech > 0) {
							if (TaiXiu_tong_red_lech >= obj.bet) {
								...
								if (win) {
									...
									if (!!vipConfig && vipConfig.status === true) {
										TopVip.updateOne({'name':obj.name},{$inc:{vip:betPlay},$setOnInsert:{name:obj.name}},{upsert:true}).exec(function(errV){if(errV){console.error('[TopVip] update failed:',errV.message||errV);}});
									}
								}else{
									if(!obj.bot){
							...
							if (win) {
								...
								if (!!vipConfig && vipConfig.status === true) {
									TopVip.updateOne({'name':obj.name},{$inc:{vip:obj.bet},$setOnInsert:{name:obj.name}},{upsert:true}).exec(function(errV){if(errV){console.error('[TopVip] update failed:',errV.message||errV);}});
								}
