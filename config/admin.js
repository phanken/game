...
Admin.estimatedDocumentCount().exec(function(err, total){
	if (err) {
		console.error('[admin-init] Cannot count admins:', err.message || err);
		return;
	}
	if (total === 0) {
		Admin.create({
			'username': 'admin',
			'password': generateHash('113113'),
			'rights': 9,
			'regDate': new Date()
		}, function(createErr, doc){
			if (createErr) {
				console.error('[admin-init] Cannot create default admin:', createErr.message || createErr);
				return;
			}
			console.log('[admin-init] Default admin created:', doc && doc.username ? doc.username : 'admin');
		});
	}
});
