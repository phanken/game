...
module.exports = function (obj) {
	new CronJob('*/3 * * * * *', function () {
		UserInfo.find({ type: true }, 'id name', function (err, blist) {
			if (err) {
				console.error('[cronchattx] UserInfo query failed:', err.message || err);
				return;
			}
				...
				Promise.all(blist.map(function (buser) {
					...
					.then(result => {
						...
						TXChat.find({}, 'value', { sort: { '_id': -1 }, skip: 15, limit: 1000 }, function (err, listchat) {
							if (err) {
								console.error('[cronchattx] TXChat query failed:', err.message || err);
								return;
							}
							if (!Array.isArray(listchat) || listchat.length === 0) {
								return;
							}
							if (listchat.length) {
